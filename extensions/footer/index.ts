...
function bgBorderLine(borderFn: (s: string) => string, content: string, width: number): string {
  const innerW = width - 2;
  const c = truncateToWidth(content, innerW);
  const cW = visibleWidth(c);
    ...
    ctx.ui.setWidget("stats-bar", (tui: any, theme: any) => {
      ...
      return {
        ...
        render(width: number): string[] {
          ...
          const thinkingText = thinkingLevel ? getThinkingText(thinkingLevel) : undefined;
          ...
          const border = (s: string) => editorRef?.borderColor(s) ?? theme.fg("borderMuted", s);
          const sep    = border("─");
          ...
          const piSeg = " " + starPi + " ";            const piW     = 1 + strWidth(icons.pi) + 1;
          const mdSeg = " " + modelStr + " ";           const mdW     = 1 + modelDisplayW + 1;
          const thSeg = thinkingText
            ? " " + theme.fg("dim", thinkingText) + " " : "";
          const thW   = thinkingText ? 1 + visibleWidth(thinkingText) + 1 : 0;

          const ctxFull   = progressBar + " " + contextLabel + theme.fg("muted", contextWindowLabel);
          const ctxFullW  = 2 + barTotal + 1 + visibleWidth(contextStr) + visibleWidth(contextWindowLabel);
          ...
          const ctxSeg  = " " + ctxFull  + " "; const ctxSegW  = 1 + ctxFullW  + 1;
          const ctxCSeg = " " + ctxCmpct + " "; const ctxCSegW = 1 + ctxCmpctW + 1;
          ...
          if (totalInput || totalOutput) {
            ...
            tokenParts.push(theme.fg("dim", t)); tokenPartsW.push(strWidth(t));
          }
          if (totalCacheRead) {
          ...
          const tokenVisW = tokenPartsW.reduce((a, b) => a + b, 0) + Math.max(0, tokenPartsW.length - 1);
          ...
          const clkSeg    = " " + timerPart + " "; const clkW = 1 + timerVisW + 1;
          ...
          const mkLeft = (useThink: boolean, cs: string, cw: number) => ({
            text: piSeg + sep + mdSeg + (useThink && thW > 0 ? sep + thSeg : "") + sep + cs,
          ...
          for (const [useThink, cs, cw, useTok] of attempts) {
            ...
            if (L.w + 1 + R.w <= usableW) { line = mkLine(L, R); break; }
          }
          if (!line) {
